...
- Canonical local test path is containerized via `docker compose -f docker-compose.dev.yml`; do not rely on host-native Postgres or a host Python virtualenv for standard repo tests
- Bootstrap the local test environment with `make test.setup`
- Run the full local backend + agent verification flow with `make test.local.full`
- One-shot backend tests: `make test` (Golang).
- Targeted backend tests: `make test PKG_TEST_PACKAGES=./pkg/workers`
- Agent unit tests: `make test.agent.unit`
- Targeted E2E tests: `make test.e2e E2E_TEST_PACKAGES=./test/e2e/workflows`
